...
app.post('/api/admin/students', verifyToken, verifyAdmin, async (req, res) => {
    ...
    if (!student_id || !full_name || !elective_id) {
...
// Reset student PIN
app.post('/api/admin/students/:id/reset-pin', verifyToken, verifyAdmin, async (req, res) => {
    const { id } = req.params;
...
// Get student attendance history (admin view)
app.get('/api/admin/students/:id/attendance', verifyToken, verifyAdmin, async (req, res) => {
    const { id } = req.params;
